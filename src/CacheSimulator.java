...
public class CacheSimulator{
	...
	public static boolean debuggingmode = false; // Didn't implement this.
	public static L2Cache L2Cache;
	...
	public static void startMSI(String[] args) throws IOException {
		...
		L2Cache L2Cachebuffer = new L2Cache(NumofSetsL2, L2Associativity);
		CacheSimulator.L2Cache = L2Cachebuffer;
		...
		System.out.println("S = " + NumofSetsL2 );
...
//			System.out.println(GlobalCycle);
			...
			for (int i = 0; i < Cores.size(); i++) {
				...
				if (!Core.CommandWaitingList.isEmpty()) {
					...
					if (command.issueCycle == GlobalCycle     //Issue command according to sequence and L1 miss
							|| command.Ready == true) {
						command.Ready = true;
						if(Core.startingCycle == -1){
							Core.startingCycle = command.issueCycle;
						}
						if (command.execute() == true) {
							...
							if (!Core.CommandWaitingList.isEmpty()) {
								...
								Core.cycleNum = CacheSimulator.GlobalCycle - Core.startingCycle;
							}
						}
					}
				}
//				System.out.println("Core["+i+"] state is");
//				Core.printCore();
			}
			if (hasPendingCommand == false)
				break;
			for(int i = 0; i < B; i++){ //think about the position of this call
				resultOfL2[i] = L2Cache.banklist.get(i).RequestTiming();
			}
			bus.nextRequest();
		...
		if(debuggingmode){
			...
			writer.println("\n\nCache status:");
			for (int i = 0; i < Cores.size(); i++) {
				Cores.get(i).L1Cache.printL1Cache();
			}

			for (int i = 0; i < B; i++) {
				L2Cache.banklist.get(i).printL2Cache();
			}
		}
		writer.close();
		traceWriter.close();
		if(!debuggingmode){
			try{
				File file = new File("./executionTrace.txt");
				if(file.delete()){
	...
	private static void InitSimulator(String Config, String tracefile) throws IOException {
		...
		while ((CommendStr = br.readLine()) != null) {
			...
			command = new Command(cycle, CoreId, rw, addr, IssueRdy);
